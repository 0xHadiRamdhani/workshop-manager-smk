...
export default function InventoryScreen() {
    ...
    useEffect(() => {
        ...
        lowStock.forEach(part => {
            if (!inventoryAlerts.find((alert: any) => alert.partId === part.id && !alert.resolved)) {
                addInventoryAlert({
    ...
    return (
        <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
            ...
            {/* Alerts Section */}
            {inventoryAlerts.filter((alert: any) => !alert.resolved).length > 0 && (
                <View style={styles.alertsSection}>
                    <Text variant="h6" color="error" weight="medium" style={styles.alertTitle}>
                        ⚠️ Alert Inventory ({inventoryAlerts.filter((alert: any) => !alert.resolved).length})
                    </Text>

                    {inventoryAlerts.filter((alert: any) => !alert.resolved).map((alert: any) => (
